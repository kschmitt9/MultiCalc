function dotani(A, B_original)
    ...
    % Add a legend
    legend([h1, h2], {'$\mathbf{a}$', '$\mathbf{b}$'}, 'Interpreter', 'latex','location','northeast');

    % Add initial text placeholder for the dot product
    t = text(-0.9, 0, 0.5, '', 'Color', 'black', 'FontSize', 14, 'Interpreter', 'latex');
    ...
    for i = 1:length(theta)
        ...
        % Update the B vector using the quiver3 handle
        h2.UData = B(1);
        h2.VData = B(2);
        h2.WData = B(3);

        % Update the text annotation for the dot product value
        set(t, 'String', sprintf('$\\mathbf{a} \\cdot \\mathbf{b}$ = %.2f', dotProd));
        ...
        if i == 1
            imwrite(imind,cm,'dotani.gif','gif', 'Loopcount',inf, 'DelayTime',0.05);
        else
            imwrite(imind,cm,'dotani.gif','gif','WriteMode','append', 'DelayTime',0.05);
        end
    end
end
