function pyth3d(v1, v2)
    ...
    P = fill3([P(1), A(1), C(1), B(1), P(1)], [P(2), A(2), C(2), B(2), P(2)], [P(3), A(3), C(3), B(3), P(3)], 'y', 'FaceAlpha', 0.3);
    hold on;

    % Plot the xy projection with solid color
    h1 = fill3(xy_proj(:, 1), xy_proj(:, 2), zeros(size(xy_proj, 1), 1), 'r', 'FaceAlpha', 0.3);

    % Plot the xz projection with solid color
    h2 = fill3(xz_proj(:, 1), zeros(size(xz_proj, 1), 1), xz_proj(:, 2), 'g', 'FaceAlpha', 0.3);

    % Plot the yz projection with solid color
    h3 = fill3(zeros(size(yz_proj, 1), 1), yz_proj(:, 1), yz_proj(:, 2), 'b', 'FaceAlpha', 0.3);
    ...
    legend([P, h1, h2, h3], {'$P$','$P_{xy}$', '$P_{xz}$', '$P_{yz}$'}, 'Interpreter', 'latex','location','northeast');
end
