function anicurvature(x_func, y_func, z_func, t)
    ...
    % Calculate the coordinates of the space curve and include axis limits with padding
    padding = 0.05;
    x = x_func(t); min_x=floor(min(x)-padding); max_x=ceil(max(x)+padding);
    y = y_func(t); min_y=floor(min(y)-padding); max_y=ceil(max(y)+padding);
    z = z_func(t); min_z=floor(min(z)-padding); max_z=ceil(max(z)+padding);
    ...
    view(125, 20); % Set the azimuth and elevation angles
    ...
    for t_idx = 3:length(t)-2
        ...
        lgd.Position = [0.1 .78 0.1 0.2]; % You can adjust these values
