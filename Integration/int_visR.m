function int_visR(r, u_range, v_range, delta)
    if nargin < 4
        delta = 10;
    end
    ...
    for i = 1:delta
        for j = 1:delta
            u_rand = u_range(1) + (i-1)*dU + rand()*dU;
            v_rand = v_range(1) + (j-1)*dV + rand()*dV;

            % Get the height for the block using the random point
            R_rand = r(u_rand, v_rand);
            z_rand = R_rand(3);
            ...
            % Use z_rand as the height for the block
            fill3_blocks(x0, y0, z0, dU, dV, z_rand, [43/255,53/255,132/255], [1,1,1],.8);
        end
    end
