function spacecurve2(x_func1, y_func1, z_func1, t1, x_func2, y_func2, z_func2, t2, frame_rate)
    ...
    % Calculate the coordinates of the space curves
    x1 = x_func1(t1);
    y1 = y_func1(t1);
    z1 = z_func1(t1);
    ...
    hold on
    h1 = plot3(x1(1), y1(1), z1(1), 'LineWidth', 2, 'color', 'red');
    h2 = plot3(x2(1), y2(1), z2(1), 'LineWidth', 2, 'color', 'green');
    xlabel('x');
    ylabel('y');
    zlabel('z');
    grid on;
    ...
    for i = 2:max_t_length
        ...
        if i <= length(t1)
            set(h1, 'XData', x1(1:i), 'YData', y1(1:i), 'ZData', z1(1:i));
        end
        if i <= length(t2)
